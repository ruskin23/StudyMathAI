...
class ContentManager:
    ...
    def add_page_text(self, book_id: int, page_number: int, page_text: str) -> PageText:
        with self.db.get_session() as session:
            page = PageText(book_id=book_id, page_number=page_number, page_text=page_text)
            session.add(page)
            try:
                session.commit()
                return page
    ...
    def get_pages_by_book(self, book_id: int) -> List[PageText]:
        with self.db.get_session() as session:
...
class ChapterManager:
    ...
    def add_segment(self, chapter_id: int, book_id: int, level: int, title: str, text: str, parent_id: Optional[int] = None) -> Optional[ChapterContent]:
        with self.db.get_session() as session:
    ...
    def get_segments_by_book(self, book_id: int) -> List[ChapterContent]:
        with self.db.get_session() as session:
...
class SlideManager:
    ...
    def add_slide(self, content_id: int, book_id: int, slide_data: dict, model_info: str = "") -> Optional[GeneratedSlide]:
        with self.db.get_session() as session:
...
class TOCManager:
    ...
    def add_entry(self, book_id: int, level: int, title: str, page_number: int, chapter_id: Optional[int] = None, parent_id: Optional[int] = None) -> TableOfContents:
        with self.db.get_session() as session:
    ...
    def get_toc_for_book(self, book_id: int) -> List[TableOfContents]:
        with self.db.get_session() as session:
...
class DatabaseManager:
    def __init__(self, db_name: str = None):
        ...
        self.books = BookManager(self.connection)
        self.content = ContentManager(self.connection)
        self.chapters = ChapterManager(self.connection)
        self.slides = SlideManager(self.connection)
        self.toc = TOCManager(self.connection)